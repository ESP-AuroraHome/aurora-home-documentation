...
async function fetchContributors(): Promise<Contributor[]> {
	...
	for (const entries of repoContributors) {
		for (const entry of entries) {
			...
			map
				.get(entry.login)
				?.repos.push({ name: entry.repo, count: entry.count });
		}
	}
				...
				<div className="flex items-start gap-3">
					...
					<div>
						...
						<p className="text-sm text-blue-800/70 dark:text-blue-200/70">
							{t("twoReposDesc")}{" "}
							<code className="px-1.5 py-0.5 bg-blue-500/20 rounded text-blue-900 dark:text-blue-200">
								aurora-home-app
							</code>{" "}
							{t("twoReposDesc2")}{" "}
							<code className="px-1.5 py-0.5 bg-blue-500/20 rounded text-blue-900 dark:text-blue-200">
					...
					<Step number={3} title={t("step3Title")}>
						...
						<ul className="space-y-2 text-sm">
							...
							<li className="flex items-center gap-2">
								...
								<span>
									{t("step3Li3")}{" "}
									<code className="px-1.5 py-0.5 bg-white/5 rounded">any</code>{" "}
									{t("step3Li3b")}
								</span>
							</li>
							<li className="flex items-center gap-2">

import { ArrowRight, Globe, Info, Server, Zap } from "lucide-react";
...
function MethodBadge({ method }: { method: string }) {
	...
	return (
		<span
			className={`text-xs font-mono px-2 py-1 rounded font-bold ${colors[method] ?? "bg-white/10 text-white"}`}
		>
			{method}
		</span>
	);
		...
		<div>
			...
			<div className="mb-4 p-4 rounded-lg bg-blue-500/10 border border-blue-500/20">
				<div className="flex items-start gap-3">
					...
					<p className="text-sm text-blue-200/70">{t("infoNote")}</p>
				</div>
			</div>
			...
			<div className="mb-12 mt-8">
				...
				<div className="mb-8 p-6 rounded-xl border border-white/10 bg-white/[0.02]">
					...
					<p className="text-sm text-neutral-400 mb-4">{t("sseDesc")}</p>
...
: keepalive`}</CodeBlock>
				...
				<div className="mb-8 p-6 rounded-xl border border-white/10 bg-white/[0.02]">
					...
					<p className="text-sm text-neutral-400 mb-4">{t("authDesc")}</p>
					<div className="grid gap-2">
						{[
							{
								path: "/api/auth/sign-in/email-otp",
								desc: t("authEndpoint1Desc"),
							},
							{
								path: "/api/auth/email-otp/verify-otp",
								desc: t("authEndpoint2Desc"),
							},
							...
							>
								<code className="text-xs text-neutral-400 flex-1 min-w-0 truncate">
									{endpoint.path}
								</code>
    ...
    "createdAt": "2024-01-15T10:35:00.000Z"
				...
				<div className="mb-8 p-6 rounded-xl border border-white/10 bg-white/[0.02]">
					...
					<p className="text-sm text-neutral-400 mb-4">
						{t("localeDesc")}{" "}
						<code className="px-1.5 py-0.5 bg-white/5 rounded">locale</code>{" "}
						{t("localeDesc2")}
					</p>
...
// Paramètres globaux
{
  ...
  "quietEnd":    7
				...
				<div className="mb-8 p-6 rounded-xl border border-yellow-500/20 bg-yellow-500/5">
					...
					<p className="text-sm text-neutral-400 mb-4">
						{t("devEndpointDesc")}{" "}
						<code className="px-1.5 py-0.5 bg-white/5 rounded">403</code>{" "}
						{t("devEndpointDesc2")}{" "}
			...
			<div className="mb-12">
				...
				<div className="grid gap-3">
					{[
						...
						>
							<code className={`font-mono font-bold text-${item.color}-400`}>
								{item.code}
							</code>
				...
				<div className="flex items-start gap-4">
					...
					<div>
						...
						<p className="text-sm text-neutral-400">
							{t("routeHandlersDesc")}{" "}
							<code className="px-1.5 py-0.5 bg-white/5 rounded">route.ts</code>{" "}
							{t("routeHandlersDesc2")}
							<code className="px-1.5 py-0.5 bg-white/5 rounded">app/api/</code>
							{t("routeHandlersDesc3")}
						</p>
					</div>
				</div>
			</div>

import { ArrowRight, Code, FileCode, TestTube } from "lucide-react";
				...
				children: [
					{
						...
						children: [
							...
							},
							{
								name: "sensor-stream",
								type: "folder",
								comment: "SSE temps réel",
							},
							{ name: "locale", type: "folder", comment: "Préférence langue" },
							{
								name: "datapoints",
								type: "folder",
								comment: "Historique capteurs + période",
							},
							{
								name: "thresholds",
								type: "folder",
								comment: "Seuils personnalisés",
							},
							{
								name: "preferences",
								type: "folder",
								comment: "Préférences notification",
							},
						],
					},
					{
						...
						children: [
							{
								name: "layout.tsx",
								type: "file",
								comment: "Vérification session",
							},
							{
								name: "page.tsx",
								type: "file",
								comment: "Dashboard principal",
							},
						],
					},
					{
					...
					{ name: "docs", type: "folder" },
					{
						name: "settings",
						type: "folder",
						comment: "Seuils & préférences alerte",
					},
				],
			},
			{
						...
						children: [
							{
								name: "components",
								type: "folder",
								comment: "LoginForm, OtpForm",
							},
							{
								name: "usecase",
								type: "folder",
								comment: "login, signInOtp, signOut",
							},
							{
								name: "repository",
								type: "folder",
								comment: "sessionRepository",
							},
						],
					},
					{
						...
						children: [
							{
								...
								type: "folder",
								comment:
									"ChartDatapoint, DatapointItem, DashboardDatapoints, IaqScore",
							},
							{
								name: "usecase",
								type: "folder",
								comment: "getInitialDataPoints",
							},
							{
								name: "repository",
								type: "folder",
								comment:
									"dataPointRepository, thresholdRepository, preferenceRepository",
							},
							{
								name: "utils",
								type: "folder",
								comment:
									"calculateChartDomain, toDataPoints, calculateIaq, downsample",
							},
						],
					},
					{
						...
						children: [
							{
								...
								type: "folder",
								comment:
									"ProfileCard, AvatarSelector, EditableFields, SignOutButton",
							},
							...
							{ name: "repository", type: "folder", comment: "userRepository" },
							{
								name: "usecase",
								type: "folder",
								comment: "getUserProfile, updateUserProfile",
							},
			...
			},
			{
				...
				children: [
					...
					{ name: "auth-client.ts", type: "file", comment: "Méthodes client" },
					{
						name: "mqtt-client.ts",
						type: "file",
						comment: "Client MQTT + parsing",
					},
					{
						name: "sensor-emitter.ts",
						type: "file",
						comment: "EventEmitter interne",
					},
					{
						name: "prisma.ts",
						type: "file",
						comment: "Client Prisma singleton",
					},
					{
						name: "otp-display.ts",
						type: "file",
						comment: "Affichage OTP sur I2C / terminal",
					},
			...
			},
			{
				...
				children: [
					{ name: "ui", type: "folder", comment: "shadcn/ui (Radix UI)" },
					{
						name: "specific",
						type: "folder",
						comment: "Composants métier (ButtonForm, etc.)",
					},
				],
			},
			{
				...
				children: [
					{
						name: "useAnimatedValue.ts",
						type: "file",
						comment: "Animation numérique easeOutCubic",
					},
					{
						name: "useSensorData.ts",
						type: "file",
						comment: "Hook SSE temps réel",
					},
					{
						name: "useTrend.ts",
						type: "file",
						comment: "Direction de tendance par capteur",
					},
				],
			},
			{
				...
				children: [
					{
						name: "schema.prisma",
						type: "file",
						comment: "Schéma base de données",
					},
					{
						name: "seedFakeData.ts",
						type: "file",
						comment: "Données de test Faker.js",
					},
					{
						name: "seed7days.ts",
						type: "file",
						comment: "7 jours de données réalistes (1pt/5min)",
					},
					{
						name: "clearData.ts",
						type: "file",
						comment: "Vider toutes les tables",
					},
				],
			},
		],
			...
			<div className="mb-12">
				...
				<div className="grid gap-3">
					{[
						...
						>
							<code className="text-sm text-green-400 min-w-[160px]">
								{name}
							</code>
...
if (result.success) {
  ...
  console.error(result.error); // string
				...
				<p className="text-sm text-neutral-500 mt-3">
					{t("usecaseNote")} (
					<code className="px-1.5 py-0.5 bg-white/5 rounded">"use server"</code>
					).
					{t("usecaseNote2")}{" "}
			...
			<div className="mb-12">
				...
				<p className="text-neutral-400 mb-4">
					{t("repositoryDesc")}{" "}
					<code className="px-1.5 py-0.5 bg-white/5 rounded">prisma.*</code>{" "}
					{t("repositoryDesc2")}
				</p>
  ...
  update: (id: string, data: Partial<User>) =>
    prisma.user.update({ where: { id }, data }),
			...
			<div className="mb-12">
				...
				<p className="text-neutral-400 mb-4">
					{t("datapointUtilsDesc")} (
					<code className="px-1.5 py-0.5 bg-white/5 rounded">"22.50 °C"</code>).
...
// Cleanup au unmount : es.close() + clearTimeout`}</CodeBlock>
			...
			<div className="mb-12">
				...
				<p className="text-neutral-400 mb-4">{t("otpDisplayDesc")}</p>
				<div className="grid gap-3 mb-4">
					{[
						...
						<div
							key={item.var}
							className="p-4 rounded-lg bg-white/[0.02] border border-white/5 text-sm"
						>
...
╚════════════════════════════════════╝`}</CodeBlock>
  ...
  "compilerOptions": {
    ...
    "paths": { "@/*": ["./*"] }  // Alias import @/
				...
				<div className="mt-6">
					<h3 className="font-semibold mb-4 text-neutral-200">
						{t("strictImposes")}{" "}
						<code className="px-1.5 py-0.5 bg-white/5 rounded text-blue-300">
							strict: true
						</code>{" "}
						...
						{[
							...
							>
								<div className="flex items-start gap-3">
									<code className="text-xs text-blue-400 min-w-[220px] pt-0.5">
										{item.flag}
									</code>
									<div>
										...
										{item.example && (
											<code className="text-xs text-neutral-600 mt-1 block">
												{item.example}
											</code>
										)}
									</div>
				...
				<div className="mt-6">
					...
					<div className="grid gap-3">
						{[
							...
							>
								<div className="flex items-start gap-3">
									<code className="text-xs text-green-400 min-w-[180px] pt-0.5">
										{item.rule}
									</code>
									<div>
										<p className="text-sm text-neutral-400">

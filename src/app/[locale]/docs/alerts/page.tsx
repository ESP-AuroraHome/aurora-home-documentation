...
function _CodeBlock({ children, title }: { children: string; title?: string }) {
	return (
	...
	return (
		<div>
			...
			<div className="mb-12">
				...
				<p className="text-neutral-400 mb-6">{t("bannerIntro")}</p>
				<div className="grid gap-4">
					<div className="p-5 rounded-xl border border-emerald-500/20 bg-emerald-500/5">
						...
						<p className="text-sm text-neutral-400">{t("healthyDesc")}</p>
					</div>
					...
					<div className="p-5 rounded-xl border border-red-500/30 bg-red-500/10">
						...
						<div className="flex gap-2 flex-wrap">
							{[
								...
								<span
									key={s.label}
									className={`text-xs px-2 py-0.5 rounded-full bg-${s.color}-500/20 text-${s.color}-300`}
								>
									{s.label}
								</span>
							))}
					...
					<div className="p-5 rounded-xl border border-white/10 bg-white/[0.02]">
						...
						<p className="text-sm text-neutral-400">{t("watchingDesc")}</p>
					</div>
				</div>
			</div>
					...
					<div className="flex items-start gap-4">
						...
						<div>
							<h3 className="font-semibold mb-2">
								{t("bellCta")}{" "}
								<strong className="text-white">{t("bellCtaStrong")}</strong>
								{t("bellCta2")}
							</h3>
					...
					{[
						{
							...
							desc: t("tabAlertsDesc"),
						},
						{
							icon: Eye,
							color: "neutral",
							title: t("tabUnseenTitle"),
							desc: t("tabUnseenDesc"),
						},
						{
							icon: CheckCheck,
							color: "green",
							title: t("tabResolvedTitle"),
							desc: t("tabResolvedDesc"),
						},
					].map((tab) => (
						<div
							key={tab.title}
						...
						>
							<div
								className={`p-2 rounded-lg bg-${tab.color}-500/10 flex-shrink-0`}
							>
						...
						<div className="flex items-start gap-3">
							...
							<div>
								...
								<p className="text-sm text-neutral-400">
									{t("markSeenDesc")}
									<strong className="text-white"> {t("markSeenAll")}</strong>{" "}
									{t("markSeenDesc2")}
								</p>
							</div>
						</div>
			...
			<div className="mb-12">
				...
				<div className="grid gap-3">
					{[
						...
						<div
							key={s.label}
							className={`p-4 rounded-xl border border-${s.color}-500/30 bg-${s.color}-500/5`}
						>
							<p className={`font-semibold text-sm text-${s.color}-300 mb-1`}>
								{s.label}
							</p>
			...
			<div className="mb-12">
				...
				<p className="text-neutral-400 mb-6">{t("thresholdsIntro")}</p>
				<div className="grid gap-4">
					{[
						{
							sensor: t("tempSensor"),
						...
						},
						{
							...
							unit: "ppm",
							color: "neutral",
							thresholds: [{ label: t("co2High"), values: t("co2HighValues") }],
						},
						{
						...
						>
							<div className="flex items-center gap-2 mb-3">
								<span
									className={`text-xs px-2 py-0.5 rounded-full bg-${s.color}-500/10 text-${s.color}-400`}
								>
									{s.unit}
								</span>
				...
				<div className="mt-4 p-4 rounded-lg bg-blue-500/10 border border-blue-500/20">
					<p className="text-sm text-blue-200/70">
						<strong className="text-blue-300">{t("suddenChangeNote")}</strong>{" "}
						{t("suddenChangeDesc")}
					</p>
				</div>
			</div>
			...
			<div className="mb-12 p-6 rounded-xl border border-white/10 bg-white/[0.02]">
				<div className="flex items-start gap-4">
					...
					<div>
						...
						<p className="text-sm text-neutral-400">
							{t("antispamDesc")}{" "}
							<strong className="text-white">{t("antispamTime")}</strong>
							{t("antispamDesc2")}
						</p>
					</div>
				</div>
				...
				<div className="grid gap-4 mb-6">
					<div className="p-5 rounded-xl border border-white/10 bg-white/[0.02]">
						<div className="flex items-start gap-4">
							...
							<div>
								<h3 className="font-semibold mb-2">
									{t("thresholdConfigTitle")}
								</h3>
								<p className="text-sm text-neutral-400 leading-relaxed">
									{t("thresholdConfigDesc")}
								</p>
							</div>
						</div>
					</div>

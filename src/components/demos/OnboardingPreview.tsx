...
// ── Stepper ────────────────────────────────────────────────────────────────────
const STEPS = [{ label: "Bienvenue" }, { label: "Avatar" }, { label: "Langue" }];
        ...
        <div key={step.label} style={{ display: "flex", alignItems: "center" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
            ...
            <div style={{ position: "relative", display: "flex", alignItems: "center", justifyContent: "center" }}>
              {i === currentStep && (
                <div style={{ position: "absolute", inset: 0, borderRadius: "50%", background: "rgba(255,255,255,0.2)", filter: "blur(6px)", transform: "scale(1.5)" }} />
              )}
              <div style={{
                position: "relative", width: 36, height: 36, borderRadius: "50%",
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 13, fontWeight: 600,
                background: i <= currentStep ? "#fff" : "rgba(255,255,255,0.05)",
                color: i <= currentStep ? "#000" : "rgba(255,255,255,0.4)",
                border: i < currentStep ? "none" : i === currentStep ? "none" : "1px solid rgba(255,255,255,0.15)",
                boxShadow: i === currentStep ? "0 0 0 4px rgba(255,255,255,0.15)" : "none",
              }}>
                {i < currentStep
                  ? <Check size={14} strokeWidth={2.5} style={{ color: "#000" }} />
                  : <span>{i + 1}</span>
                }
              </div>
            </div>
            ...
            <span style={{
              ...
              color: i === currentStep ? "#fff" : i < currentStep ? "rgba(255,255,255,0.6)" : "rgba(255,255,255,0.3)",
            }}>
              {step.label}
            </span>
          </div>
          ...
          {/* Connector */}
          {i < STEPS.length - 1 && (
            <div style={{ position: "relative", width: 48, height: 1, margin: "0 8px 20px", overflow: "hidden" }}>
              <div style={{ position: "absolute", inset: 0, background: "rgba(255,255,255,0.15)", borderRadius: 2 }} />
              <div style={{ position: "absolute", inset: 0, background: "#fff", borderRadius: 2, transformOrigin: "left", transform: `scaleX(${i < currentStep ? 1 : 0})`, transition: "transform 0.4s ease" }} />
            </div>
          )}
        </div>
      ))}
...
// ── Onboarding layout ──────────────────────────────────────────────────────────
function OnboardingLayout({ children, step }: { children: React.ReactNode; step: number }) {
  return (
    <div style={{
      ...
      />
      {/* Content */}
      <div style={{ position: "relative", zIndex: 10, width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
        {/* Logo */}
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/logo-black.png" alt="Aurora Home" style={{ width: 48, height: 48, objectFit: "contain" }} />
...
// ── Step 0: Welcome ────────────────────────────────────────────────────────────
function StepWelcome({ name, onChange, onNext }: { name: string; onChange: (v: string) => void; onNext: () => void }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div>
        <h1 style={{ color: "#fff", fontSize: 20, fontWeight: 600, margin: "0 0 8px" }}>
          Bienvenue sur Aurora Home
        </h1>
        <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 12, lineHeight: 1.6, margin: 0 }}>
          Commençons par personnaliser votre espace. Comment souhaitez-vous être appelé ?
        </p>
      </div>

      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        <label style={{ color: "rgba(255,255,255,0.7)", fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em" }}>
          Votre prénom
        </label>
        <input
          type="text"
      ...
      <button
        ...
        style={{
          ...
          border: "none", borderRadius: 12, padding: "12px 0", cursor: name.trim() ? "pointer" : "not-allowed",
...
// ── Step 1: Avatar ─────────────────────────────────────────────────────────────
function StepAvatar({
  avatarOptions, selected, onSelect, onNext, onBack,
}: {
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div>
        <button
          type="button"
          onClick={onBack}
          style={{ display: "flex", alignItems: "center", gap: 4, color: "rgba(255,255,255,0.4)", fontSize: 11, background: "none", border: "none", cursor: "pointer", padding: 0, marginBottom: 12 }}
        >
          <ArrowLeft size={12} style={{ color: "rgba(255,255,255,0.4)" }} />
          <span style={{ color: "rgba(255,255,255,0.4)" }}>Retour</span>
        </button>
        <h2 style={{ color: "#fff", fontSize: 18, fontWeight: 600, margin: "0 0 6px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, maxHeight: 240, overflowY: "auto", paddingRight: 2 }}>
          ...
          return (
            <button
              ...
              style={{
                ...
                border: isSelected ? "2px solid #fff" : "2px solid rgba(255,255,255,0.10)",
            ...
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={option.url} alt={option.style} style={{ width: "100%", height: "100%", objectFit: "cover", padding: 4, boxSizing: "border-box" }} />
              {isSelected && (
...
// ── Step 2: Locale ─────────────────────────────────────────────────────────────
function StepLocale({
  locale, onChange, onFinish, onBack,
}: {
  locale: "fr" | "en";
        ...
        {languages.map((lang) => (
          <button
            ...
            style={{
              ...
              border: locale === lang.code ? "2px solid #fff" : "2px solid rgba(255,255,255,0.10)",
              background: locale === lang.code ? "rgba(255,255,255,0.15)" : "transparent",
              cursor: "pointer", transition: "all 0.2s ease",
            }}
          >
            <span style={{ color: "#fff", fontWeight: 500, fontSize: 14 }}>{lang.label}</span>
            {locale === lang.code && (
...
// ── Success screen ─────────────────────────────────────────────────────────────
function SuccessScreen({ selectedAvatar, name, onRestart }: { selectedAvatar: string | null; name: string; onRestart: () => void }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 20, padding: "40px 24px", textAlign: "center" }}>
      <div style={{ position: "relative", display: "inline-block" }}>
        {selectedAvatar ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={selectedAvatar} alt="avatar" style={{ width: 72, height: 72, borderRadius: "50%", border: "3px solid rgba(255,255,255,0.2)" }} />
        ) : (
          <div style={{ width: 72, height: 72, borderRadius: "50%", background: "rgba(255,255,255,0.1)", border: "3px solid rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <span style={{ color: "#fff", fontSize: 24 }}>👤</span>
          </div>
        )}
        <div style={{ position: "absolute", bottom: -4, right: -4, width: 24, height: 24, borderRadius: "50%", background: "#22c55e", display: "flex", alignItems: "center", justifyContent: "center", border: "2px solid rgba(0,0,0,0.5)" }}>
      ...
      <button
        ...
        style={{
          ...
          background: "rgba(255,255,255,0.12)", color: "#fff", fontSize: 13, fontWeight: 500,
          border: "1px solid rgba(255,255,255,0.15)", borderRadius: 12, padding: "10px 20px",
          cursor: "pointer",
        }}
      >
        Recommencer la démo
  ...
  const avatarOptions = useMemo(
    () =>
      AVATAR_STYLES.map((style) => ({
        ...
        url: createAvatar(style.generator as any, { seed: name || "aurora", size: 128 }).toDataUri(),
      })),
    [name],
  );
      ...
      <div style={{
        ...
        {finished ? (
          <div style={{ position: "relative", zIndex: 10, width: "100%" }}>
            <SuccessScreen selectedAvatar={selectedAvatar} name={name} onRestart={handleRestart} />
          </div>
        ) : (
          <div style={{ position: "relative", zIndex: 10, width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
            {/* Logo */}
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="/logo-black.png" alt="Aurora Home" style={{ width: 44, height: 44, objectFit: "contain" }} />

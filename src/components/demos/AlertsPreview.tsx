...
/**
 * Displays a single alert card with severity styling, suggestions, and action buttons.
 *
 * @param alert - The alert data to display
 * @param onRead - Callback fired when the user marks the alert as read
 * @param onResolve - Callback fired when the user resolves the alert
 */
function AlertCard({
	alert,
	onRead,
		...
		>
			...
			<div style={{ flex: 1, minWidth: 0 }}>
				...
				>
					<p
						style={{
							color: isResolved ? "rgba(255,255,255,0.4)" : "#fff",
							fontSize: 13,
							fontWeight: 500,
							lineHeight: "1.4",
							margin: 0,
...
/**
 * Full-screen notification panel with tabbed views for all, unread, and resolved alerts.
 *
 * @param alerts - Current list of alerts to display
 * @param onRead - Callback to mark a single alert as read
 * @param onResolve - Callback to mark a single alert as resolved
 * @param onResolveAll - Callback to mark all alerts as resolved
					...
					{tabs.map((t) => (
						<button
							...
							style={{
								...
								fontWeight: 500,
								background:
									tab === t.key ? "rgba(255,255,255,0.15)" : "transparent",
...
/**
 * Dashboard header banner showing a healthy state, active alerts, or monitoring status.
 *
 * @param alerts - Current list of alerts used to determine the displayed state
 * @param onDismiss - Callback fired when the user dismisses a specific alert banner
 */
	...
	if (isHealthy) {
		return (
			...
			>
				...
				<div style={{ flex: 1 }}>
					<p
						style={{ color: "#fff", fontSize: 13, fontWeight: 500, margin: 0 }}
					>
		...
		return (
			...
			>
				...
				<div>
					...
					>
						{alerts.filter((a) => !a.resolvedAt).length} anomalie(s) active(s)
						en cours
					</p>
				</div>
			</div>
			...
			{sorted.map((alert) => {
				...
				return (
					...
					>
						<AlertTriangle
							size={14}
							style={{ color: sev.iconColor, flexShrink: 0, marginTop: 1 }}
						/>
						<div style={{ flex: 1, minWidth: 0 }}>
							<div
								style={{
...
/**
 * iPhone 14 hardware mockup container with status bar and home indicator.
 *
 * @param children - Screen content rendered inside the scrollable area
 * @param overlay - Optional absolute overlay rendered above the scroll area (e.g. a panel)
 */
...
/**
 * Interactive alerts demo showing the dashboard banner and notification panel inside an iPhone mockup.
 */
export default function AlertsPreview() {
	...
	const bellBadgeColor = alerts.some(
		(a) => !a.read && !a.resolvedAt && a.severity === "CRITICAL",
	)
		? "#ef4444"
	...
	const handleResolve = (id: string) =>
		setAlerts((prev) =>
			prev.map((a) =>
				a.id === id
					? { ...a, resolvedAt: new Date().toISOString(), read: true }
					: a,
			),
		);
					...
					>
						...
						>
							<Bell
								size={20}
								style={{
									color: unreadCount > 0 ? "#fff" : "rgba(255,255,255,0.6)",
								}}
							/>
							{unreadCount > 0 && (
								<span

...
// ── Shared auth layout (background + centered content) ────────────────────────
function AuthLayout({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "var(--font-geist-sans), system-ui, -apple-system, sans-serif" }}>
      {/* Background */}
      ...
      />
      {/* Content */}
      <div style={{ position: "relative", zIndex: 10, width: "100%", display: "flex", flexDirection: "column", alignItems: "center", gap: 0 }}>
        {children}
      </div>
    </div>
  );
...
// ── Card ──────────────────────────────────────────────────────────────────────
// Exact replica: bg-black/20 backdrop-blur-md border-0 rounded-3xl shadow-lg
...
// ── LiquidGlass button ────────────────────────────────────────────────────────
function LiquidGlassButton({ children, onClick, disabled }: { children: React.ReactNode; onClick?: () => void; disabled?: boolean }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
// ── Input ─────────────────────────────────────────────────────────────────────
// bg-white/10 backdrop-blur-sm border-white/20 text-white placeholder:text-white/50
...
// ── OTP slots (6 individual inputs) ──────────────────────────────────────────
// Exact replica: h-12 w-12 rounded-lg bg-white/10 border-white/20 text-white text-lg gap-1
      ...
      {digits.map((digit, i) => (
        <input
          ...
          style={{
            ...
            background: digit ? "rgba(255,255,255,0.15)" : "rgba(255,255,255,0.10)",
            backdropFilter: "blur(4px)",
    ...
    <AuthLayout>
      ...
      <AuthCard>
        ...
        <div style={{ padding: 24, paddingBottom: 0 }}>
          <h1 style={{ color: "#fff", fontSize: 22, fontWeight: 600, textAlign: "center", margin: 0 }}>
            Connexion
          </h1>
        </div>
        ...
        <div style={{ padding: "16px 24px 24px", display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            <label style={{ color: "#fff", fontSize: 13, fontWeight: 500 }}>Email</label>
            <TextInput
              type="email"
              value={email}
  ...
  return (
    <AuthLayout>
      ...
      <AuthCard>
        {/* CardHeader */}
        <div style={{ padding: 24, paddingBottom: 0 }}>
          <h1 style={{ color: "#fff", fontSize: 22, fontWeight: 600, textAlign: "center", margin: 0, marginBottom: 10 }}>
            Vérification
          </h1>
          <p style={{ color: "rgba(255,255,255,0.6)", fontSize: 13, textAlign: "center", margin: 0, fontWeight: 300, lineHeight: "1.5" }}>
            Un code à 6 chiffres a été envoyé à{" "}
            <span style={{ color: "rgba(255,255,255,0.9)", fontWeight: 500 }}>{email}</span>
          </p>
        </div>
        {/* CardContent */}
        <div style={{ padding: "16px 24px 24px", display: "flex", flexDirection: "column", gap: 20 }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
            <label style={{ color: "#fff", fontSize: 13, fontWeight: 500 }}>Code de vérification</label>
          ...
          <button
            type="button"
            onClick={onBack}
            style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.4)", fontSize: 12, fontFamily: "inherit", textAlign: "center" }}
          >
    ...
    <AuthLayout>
      ...
      <AuthCard>
        <div style={{ padding: "28px 24px", display: "flex", flexDirection: "column", alignItems: "center", gap: 16, textAlign: "center" }}>
          <div style={{ width: 56, height: 56, borderRadius: "50%", background: "rgba(52,211,153,0.15)", border: "1px solid rgba(52,211,153,0.3)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div>
            <p style={{ color: "#fff", fontSize: 17, fontWeight: 600, margin: 0, marginBottom: 6 }}>Connexion réussie !</p>
            <p style={{ color: "rgba(255,255,255,0.5)", fontSize: 13, margin: 0 }}>Vous êtes maintenant connecté à AuroraHome.</p>
          </div>
          <button
            type="button"
            onClick={onReset}
            style={{ marginTop: 4, background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.4)", fontSize: 12, fontFamily: "inherit" }}
          >
            Recommencer la démo
          </button>
        </div>
...
export default function AuthPreview() {
  ...
  return (
    <IPhoneShell>
      ...
      {screen === "success" && (
        <SuccessScreen onReset={() => { setEmail(""); setScreen("login"); }} />
      )}
    </IPhoneShell>
  );
}

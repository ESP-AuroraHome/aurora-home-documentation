...
export default function ProfilePreview() {
  ...
  const avatarOptions = useMemo(
    () =>
      AVATAR_STYLES.map((style) => ({
        ...
        url: createAvatar(style.generator as any, { seed: SEED, size: 128 }).toDataUri(),
      })),
    [],
  );
  ...
  const [editingField, setEditingField] = useState<"name" | "email" | "locale" | null>(null);
  const [editNameVal, setEditNameVal] = useState(name);
  const [saveSuccess, setSaveSuccess] = useState(false);

  const hasChanges = selectedAvatar !== savedAvatar || name !== savedName || locale !== savedLocale || email !== savedEmail;
  ...
  return (
    <IPhoneShell>
      <div style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden" }}>
        ...
        {/* Scrollable content */}
        <div style={{ position: "relative", zIndex: 10, height: "100%", overflowY: "auto", padding: "16px 16px 24px", display: "flex", flexDirection: "column", gap: 12, fontFamily: ff }}>
            ...
            {pickingAvatar ? (
              ...
              <div style={{ padding: "20px 20px 20px" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                  <span style={{ color: "#fff", fontSize: 13, fontWeight: 500 }}>Choisir un avatar</span>
                  <button
                    type="button"
                    onClick={() => setPickingAvatar(false)}
                    style={{ background: "none", border: "none", cursor: "pointer", padding: 2, color: "rgba(255,255,255,0.4)" }}
                  >
                  ...
                  {avatarOptions.map((opt) => {
                    ...
                    return (
                      <button
                        key={opt.style}
                        type="button"
                        onClick={() => { setSelectedAvatar(opt.url); setPickingAvatar(false); }}
                        style={{
                          position: "relative", aspectRatio: "1/1", borderRadius: "50%", overflow: "hidden",
                          border: isSel ? "2px solid #fff" : "2px solid rgba(255,255,255,0.10)",
                          boxShadow: isSel ? "0 0 0 2px rgba(255,255,255,0.3)" : "none",
                          transform: isSel ? "scale(1.05)" : "scale(1)",
                          transition: "all 0.15s ease", background: "rgba(255,255,255,0.05)",
                          cursor: "pointer", padding: 0,
                        }}
                      >
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={opt.url} alt={opt.style} style={{ width: "100%", height: "100%", objectFit: "cover", padding: 2, boxSizing: "border-box" }} />
                        {isSel && (
              ...
              <div style={{ padding: "28px 20px", display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
                ...
                >
                  ...
                  <div style={{ position: "absolute", inset: 0, borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0)", transition: "background 0.15s" }}>
                    <SquarePen size={18} style={{ color: "rgba(255,255,255,0)", transition: "color 0.15s" }} />
                  </div>
                  {/* Edit hint overlay — always slightly visible */}
                  <div style={{ position: "absolute", bottom: 0, right: 0, width: 24, height: 24, borderRadius: "50%", background: "rgba(0,0,0,0.6)", border: "1.5px solid rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <SquarePen size={11} style={{ color: "rgba(255,255,255,0.7)" }} />
                  </div>
                </button>
                ...
                {editingField === "name" ? (
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4, width: "100%", maxWidth: 200 }}>
                    <input
                      ...
                      onBlur={() => { setName(editNameVal.trim() || name); setEditingField(null); }}
                      onKeyDown={(e) => { if (e.key === "Enter") { setName(editNameVal.trim() || name); setEditingField(null); } }}
                      autoFocus
                      style={{
                        width: "100%", boxSizing: "border-box",
                        background: "rgba(255,255,255,0.10)", border: "1px solid rgba(255,255,255,0.20)",
                        borderRadius: 10, padding: "6px 12px", color: "#fff", fontSize: 15,
                        textAlign: "center", outline: "none", fontFamily: ff,
                      }}
                    />
                  </div>
                ) : (
                  <button
                    type="button"
                    onClick={() => { setEditNameVal(name); setEditingField("name"); }}
                    style={{ display: "flex", alignItems: "center", gap: 6, background: "none", border: "none", cursor: "pointer", padding: 0 }}
                  >
                    <span style={{ color: "#fff", fontSize: 17, fontWeight: 600 }}>{name}</span>
                    <SquarePen size={13} style={{ color: "rgba(255,255,255,0.22)" }} />
                  </button>
                )}
              </div>
            )}
          ...
          <div style={{ background: "rgba(0,0,0,0.20)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", borderRadius: 24, overflow: "hidden" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 20px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
              <span style={{ color: "rgba(255,255,255,0.50)", fontSize: 13, width: 72, flexShrink: 0 }}>Email</span>
              {editingField === "email" ? (
                <div style={{ flex: 1, minWidth: 0 }}>
                  <input
                    ...
                    onBlur={() => { setEmail(editEmailVal.trim() || email); setEditingField(null); }}
                    onKeyDown={(e) => { if (e.key === "Enter") { setEmail(editEmailVal.trim() || email); setEditingField(null); } }}
                    autoFocus
                    style={{
                      width: "100%", boxSizing: "border-box",
                      background: "rgba(255,255,255,0.10)", border: "1px solid rgba(255,255,255,0.20)",
                      borderRadius: 8, padding: "4px 10px", color: "#fff", fontSize: 13,
                ...
                <div style={{ display: "flex", flex: 1, alignItems: "center", justifyContent: "flex-end", gap: 8, minWidth: 0 }}>
                  <span style={{ color: "#fff", fontSize: 13, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{email}</span>
                  <button
                    type="button"
                    onClick={() => { setEditEmailVal(email); setEditingField("email"); }}
                    style={{ background: "none", border: "none", cursor: "pointer", padding: 0, flexShrink: 0 }}
                  >
                    <SquarePen size={13} style={{ color: "rgba(255,255,255,0.25)" }} />
                  </button>
                </div>
              )}
            </div>
            ...
            {/* Language row */}
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 20px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
              <span style={{ color: "rgba(255,255,255,0.50)", fontSize: 13, width: 72, flexShrink: 0 }}>Langue</span>
              {editingField === "locale" ? (
                <div style={{ flex: 1, display: "flex", justifyContent: "flex-end" }}>
                  <select
                    value={locale}
                    onChange={(e) => { setLocale(e.target.value as "fr" | "en"); setEditingField(null); }}
                  ...
                  >
                    <option value="fr" style={{ background: "#1a1a1e" }}>Français</option>
                    <option value="en" style={{ background: "#1a1a1e" }}>English</option>
                  </select>
                </div>
              ) : (
                <div style={{ display: "flex", flex: 1, alignItems: "center", justifyContent: "flex-end", gap: 8 }}>
                  <span style={{ color: "#fff", fontSize: 13 }}>{locale === "fr" ? "Français" : "English"}</span>
                  <button
                    type="button"
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 20px" }}>
              <span style={{ color: "rgba(255,255,255,0.50)", fontSize: 13, width: 72, flexShrink: 0 }}>Vérification</span>
          ...
          {hasChanges && (
            <div style={{ background: "rgba(0,0,0,0.20)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", borderRadius: 24, padding: 14 }}>
              <button
                type="button"
                onClick={handleSave}
                style={{
                  ...
                  borderRadius: 14, padding: "11px 0", cursor: "pointer",
                  color: "#fff", fontSize: 14, fontWeight: 500, fontFamily: ff,
          ...
          {/* ── Sign out card ── */}
          <div style={{ background: "rgba(0,0,0,0.20)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", borderRadius: 24, padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <p style={{ color: "#fff", fontSize: 13, fontWeight: 500, margin: 0 }}>Se déconnecter</p>
              <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 11, margin: "3px 0 0" }}>Fermer la session en cours</p>
            </div>
            <button
              type="button"
              style={{
                ...
                color: "rgba(255,255,255,0.65)", fontSize: 13, cursor: "pointer", fontFamily: ff,
              }}
            >

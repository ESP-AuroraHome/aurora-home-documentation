...
type ChartPoint = { value: number; timestamp: number; label: string; time: string; date: string };
...
const PERIOD_POINTS: Record<Period, number> = { live: 20, "1h": 24, "6h": 30, "24h": 36, "7j": 42 };
const PERIOD_VARIANCE: Record<Period, number> = { live: 0.01, "1h": 0.02, "6h": 0.04, "24h": 0.06, "7j": 0.08 };
...
function calcDomain(type: string, values: number[]): [number, number] {
  if (values.length === 0) {
    const defaults: Record<string, [number, number]> = {
      TEMPERATURE: [0, 30], HUMIDITY: [0, 100], PRESSURE: [950, 1050], CO2: [300, 2000], LIGHT: [0, 1000],
    };
    return defaults[type] ?? [0, 100];
  }
  const mn = Math.min(...values), mx = Math.max(...values);
  const margin = (mx - mn) * 0.1 || 1;
  let lo = mn - margin, hi = mx + margin;
  if (type === "HUMIDITY") { lo = Math.max(lo, 0); hi = Math.min(hi, 100); }
...
function AreaChart({
  ...
  return (
    ...
    >
      ...
      {showAxes && labels && labels.map((label, i) => {
        if (!label) return null;
        const x = pad + (i / (labels.length - 1)) * (W - pad);
        return (
          <text key={i} x={x} y={H + 14} textAnchor="middle" fill="rgba(255,255,255,0.35)" fontSize={10}>
            {label}
          </text>
        );
      })}
...
const SENSORS = [
  ...
  },
  {
    ...
    unit: "hPa",
    trend: "stable" as const,
    data: [1013.0, 1013.1, 1013.0, 1013.2, 1013.1, 1013.3, 1013.2, 1013.1, 1013.2, 1013.2],
  },
  {
    type: "CO2",
    label: "CO2",
...
function IconDataType({ type, size = 24 }: { type: string; size?: number }) {
  ...
  switch (type) {
    case "TEMPERATURE":
      return <Thermometer strokeWidth={2} size={size} className={cls} style={{ background: "#facc15" }} />;
    case "HUMIDITY":
      return <Droplet strokeWidth={1} size={size} className={cls} style={{ background: "#60a5fa" }} />;
    case "PRESSURE":
      return <Gauge strokeWidth={1} size={size} className={cls} style={{ background: "#4ade80" }} />;
    case "CO2":
      return <Cloud strokeWidth={1} size={size} className={cls} style={{ background: "#9ca3af" }} />;
    case "LIGHT":
      return <Sun strokeWidth={1} size={size} className={cls} style={{ background: "#fb923c" }} />;
    default:
      return null;
  }
}
...
function TrendIndicator({ trend }: { trend: "up" | "down" | "stable" }) {
  if (trend === "up")
    return (
      <span style={{ display: "flex", alignItems: "center", gap: 3, fontSize: 12, color: "#fdba74" }}>
        <TrendingUp size={14} />
        En hausse
      </span>
    );
  if (trend === "down")
    return (
      <span style={{ display: "flex", alignItems: "center", gap: 3, fontSize: 12, color: "#6ee7b7" }}>
        <TrendingDown size={14} />
        En baisse
      </span>
    );
  return (
    <span style={{ display: "flex", alignItems: "center", gap: 3, fontSize: 12, color: "rgba(255,255,255,0.3)" }}>
      <Minus size={14} />
      Stable
    </span>
  );
      ...
      <div style={{ display: "flex", width: "100%", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8, minWidth: 0, flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            <p style={{ fontWeight: 600, fontSize: 16, color: "#fff", margin: 0, whiteSpace: "nowrap" }}>{sensor.label}</p>
          </div>
          <p style={{ color: "#cbd5e1", fontSize: 12, margin: 0 }}>Jeu. 24 Mar 2026</p>
        </div>
        {/* Right */}
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0 }}>
          <p style={{ fontWeight: 700, fontSize: 22, color: "#fff", margin: 0, whiteSpace: "nowrap" }}>
...
// ── DrawerChart (Recharts — pixel-perfect from app) ───────────────────────────
function DrawerChart({ data, type, unit }: { data: ChartPoint[]; type: string; unit: string }) {
  const domain = calcDomain(type, data.map((d) => d.value));
  const uid = useMemo(() => `dg-${Math.random().toString(36).slice(2, 8)}`, []);

  return (
    <ResponsiveContainer width="100%" height={230}>
      <RechartsAreaChart data={data} margin={{ top: 10, right: 8, left: 0, bottom: 4 }}>
        <defs>
        ...
        <Tooltip
          content={({ active, payload }) => {
            ...
            return (
              <div style={{ background: "rgba(0,0,0,0.90)", backdropFilter: "blur(4px)", border: "1px solid rgba(255,255,255,0.20)", borderRadius: 8, padding: "6px 10px", boxShadow: "0 4px 16px rgba(0,0,0,0.4)" }}>
                <p style={{ color: "#fff", fontSize: 11, margin: "0 0 2px" }}>{pt.date} {pt.time}</p>
...
function SensorDetailDrawer({ sensor, onClose }: { sensor: (typeof SENSORS)[0]; onClose: () => void }) {
  const [period, setPeriod] = useState<Period>("live");
  const chartData = useMemo(() => generateTimeSeries(sensor.value, period), [sensor.value, period]);

  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 50, display: "flex", flexDirection: "column", justifyContent: "flex-end" }}>
      {/* Scrim */}
      <div onClick={onClose} style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(2px)", WebkitBackdropFilter: "blur(2px)" }} />

      {/* Bottom sheet */}
      <div style={{ position: "relative", zIndex: 1, background: "rgba(0,0,0,0.20)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderRadius: "24px 24px 0 0", border: "1px solid rgba(255,255,255,0.08)", borderBottom: "none" }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", padding: "8px 20px 10px" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <p style={{ color: "#fff", fontWeight: 600, fontSize: 18, margin: 0 }}>{sensor.label}</p>
            </div>
            <p style={{ color: "#cbd5e1", fontSize: 13, margin: 0 }}>Jeu. 24 Mar 2026</p>
          </div>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 3 }}>
            <p style={{ fontWeight: 700, fontSize: 22, color: "#fff", margin: 0, whiteSpace: "nowrap" }}>
              {sensor.value.toFixed(2)}
              <span style={{ fontWeight: 400, fontSize: 14, color: "rgba(255,255,255,0.6)", marginLeft: 4 }}>{sensor.unit}</span>
            </p>
            <TrendIndicator trend={sensor.trend} />
          </div>
        </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
            {DRAWER_PERIODS.map((p) => (
              <button
                ...
                style={{
                  padding: "4px 7px", borderRadius: 20, fontSize: 11,
                  border: period === p.value ? "1px solid rgba(255,255,255,0.20)" : "1px solid transparent",
                  background: period === p.value ? "rgba(255,255,255,0.20)" : "rgba(255,255,255,0.05)",
            ...
            <button
              type="button"
              style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 4, padding: "4px 8px", borderRadius: 20, fontSize: 11, border: "1px solid transparent", background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.50)", cursor: "pointer", fontFamily: "inherit", flexShrink: 0 }}
            >
              <Download size={10} />
              Export
            </button>
  ...
  return (
    ...
    >
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
        ...
        <p style={{ color: "#fff", fontSize: 14, fontWeight: 500, margin: 0 }}>Bon</p>
      </div>
      <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 4, minWidth: 0 }}>
        <div style={{ width: "100%", height: 4, background: "rgba(255,255,255,0.1)", borderRadius: 2, overflow: "hidden" }}>
          <div style={{ width: `${score}%`, height: "100%", background: "#38bdf8", borderRadius: 2 }} />
        </div>
        <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 12, margin: 0 }}>La qualité de l&apos;air est satisfaisante</p>
      </div>
      <span style={{ color: "#7dd3fc", fontWeight: 700, fontSize: 14, flexShrink: 0 }}>
        {score}
        <span style={{ color: "rgba(255,255,255,0.3)", fontWeight: 400, fontSize: 11 }}>/100</span>
      </span>
    </div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ color: "#fff", fontSize: 14, fontWeight: 500, margin: 0 }}>Votre maison est en bonne santé</p>
        <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 12, margin: "2px 0 0" }}>Tous les capteurs sont dans les normes</p>
      </div>
    </div>
  );
}
  ...
  return (
    <header style={{ display: "flex", alignItems: "center", gap: 12, width: "100%", padding: "8px 0" }}>
      <button
        type="button"
        style={{
          width: 56, height: 56, borderRadius: "50%", flexShrink: 0, padding: 0,
          border: "none", cursor: "pointer", overflow: "hidden", background: "transparent",
        }}
      >
      ...
      <div style={{ flex: 1 }}>
        <p style={{ color: "#e2e8f0", fontSize: 14, margin: 0, lineHeight: "150%" }}>Bonjour,</p>
        <p style={{ color: "#fff", fontWeight: 600, fontSize: 22, margin: 0 }}>Jean Dupont</p>
      </div>
      <button
        type="button"
        style={{ padding: 8, borderRadius: 12, background: "transparent", border: "none", cursor: "pointer" }}
      >
...
// ── iPhone mockup shell ───────────────────────────────────────────────────────
function IPhoneShell({ children, drawer }: { children: React.ReactNode; drawer?: React.ReactNode }) {
  return (
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
              ...
              <svg width="24" height="12" viewBox="0 0 24 12" fill="none">
                ...
                <rect x="21.5" y="3.5" width="2" height="5" rx="1.5" fill="white" fillOpacity="0.4" />

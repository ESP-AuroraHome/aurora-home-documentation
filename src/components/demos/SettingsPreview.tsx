...
// ── Constants ─────────────────────────────────────────────────────────────────
const DATA_TYPES: SensorType[] = ["TEMPERATURE", "HUMIDITY", "PRESSURE", "CO2", "LIGHT"];

const SENSOR_META: Record<SensorType, { label: string; unit: string; iconBg: string; Icon: React.ElementType }> = {
  TEMPERATURE: { label: "Température", unit: "°C",  iconBg: "#facc15", Icon: Thermometer },
...
// ── Switch (replicates Radix UI Switch from the app) ──────────────────────────
...
// ── SeveritySelect (replicates Radix Select from the app) ─────────────────────
...
// ── NumberInput (exact replica: − / value / + pill) ───────────────────────────
function NumberInput({
  ...
  return (
    <div style={{
      ...
      <span style={{
        padding: "0 4px", color: "#fff", fontSize: 11,
        fontWeight: 500, minWidth: 28, textAlign: "center",
        userSelect: "none", whiteSpace: "nowrap",
      }}>
        {display}
        <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 10, marginLeft: 1 }}>{unit}</span>
      </span>
      <button
        type="button"
        onClick={() => onChange(String(Number((numeric + step).toFixed(2))))}
...
// ── SaveButton (liquid-glass variant from the app) ────────────────────────────
function SaveButton({ onClick }: { onClick: () => void }) {
  return (
    <div style={{ background: "rgba(0,0,0,0.20)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", borderRadius: 24, padding: 16 }}>
      <button
        type="button"
        onClick={onClick}
        style={{
...
function NotificationPrefsCard() {
  const [prefs, setPrefs] = useState<Record<SensorType, { enabled: boolean; minSeverity: Severity }>>(
    Object.fromEntries(DATA_TYPES.map((t) => [t, { enabled: true, minSeverity: "WARNING" as Severity }])) as Record<SensorType, { enabled: boolean; minSeverity: Severity }>,
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      {/* Section label */}
      <h2 style={{ color: "rgba(255,255,255,0.6)", fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", margin: 0, padding: "0 4px" }}>
        Préférences de notification
      </h2>

      {/* Per-sensor rows */}
      <div style={{ background: "rgba(0,0,0,0.20)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", borderRadius: 24, overflow: "hidden" }}>
          ...
          return (
            <div
              ...
              style={{
                ...
                padding: "10px 16px",
                borderBottom: i < DATA_TYPES.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none",
              }}
            >
              {/* Icon */}
              ...
              {/* Label */}
              <span style={{ color: "#fff", fontSize: 13, flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{meta.label}</span>
              {/* Min severity select */}
              <div style={{ width: 80, flexShrink: 0 }}>
                <SeveritySelect
                  value={pref.minSeverity}
                  onChange={(v) => { setPrefs((p) => ({ ...p, [type]: { ...p[type], minSeverity: v as Severity } })); setChanged(); }}
              ...
              <Switch
                ...
                onCheckedChange={(v) => { setPrefs((p) => ({ ...p, [type]: { ...p[type], enabled: v } })); setChanged(); }}
              />
            </div>
          );
        })}
      ...
      {/* Quiet hours */}
      <div style={{ background: "rgba(0,0,0,0.20)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", borderRadius: 24, overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: quietEnabled ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
          <div>
            <p style={{ color: "#fff", fontSize: 13, fontWeight: 600, margin: 0 }}>Heures silencieuses</p>
            <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 11, margin: "2px 0 0" }}>Aucune alerte pendant ces heures</p>
          </div>
          <Switch checked={quietEnabled} onCheckedChange={(v) => { setQuietEnabled(v); setChanged(); }} />
        </div>
        {quietEnabled && (
          <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, flex: 1 }}>
              <span style={{ color: "rgba(255,255,255,0.5)", fontSize: 11, flexShrink: 0 }}>De</span>
              <select style={{ flex: 1, height: 32, background: "rgba(255,255,255,0.10)", border: "1px solid rgba(255,255,255,0.10)", color: "#fff", borderRadius: 8, padding: "0 8px", fontSize: 11, fontFamily: "inherit", outline: "none" }}>
                {Array.from({ length: 24 }, (_, h) => (
                  <option key={h} value={h} style={{ background: "#111" }}>{String(h).padStart(2, "0")}:00</option>
                ))}
              </select>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 6, flex: 1 }}>
              <span style={{ color: "rgba(255,255,255,0.5)", fontSize: 11, flexShrink: 0 }}>À</span>
              <select defaultValue="7" style={{ flex: 1, height: 32, background: "rgba(255,255,255,0.10)", border: "1px solid rgba(255,255,255,0.10)", color: "#fff", borderRadius: 8, padding: "0 8px", fontSize: 11, fontFamily: "inherit", outline: "none" }}>
                {Array.from({ length: 24 }, (_, h) => (
                  <option key={h} value={h} style={{ background: "#111" }}>{String(h).padStart(2, "0")}:00</option>
                ))}
              </select>
...
// ── ThresholdsCard ────────────────────────────────────────────────────────────
type LocalThreshold = { highValue: string; highSeverity: Severity; lowValue: string; lowSeverity: Severity };

function ThresholdsCard() {
  const [values, setValues] = useState<Record<SensorType, LocalThreshold>>(
    Object.fromEntries(DATA_TYPES.map((t) => {
      const d = DEFAULTS[t];
      return [t, { highValue: d.high != null ? String(d.high) : "", highSeverity: "WARNING" as Severity, lowValue: d.low != null ? String(d.low) : "", lowSeverity: "WARNING" as Severity }];
    })) as Record<SensorType, LocalThreshold>,
  );
  const [saved, setSaved] = useState(true);

  const update = (type: SensorType, field: keyof LocalThreshold, val: string) => {
  ...
  const reset = (type: SensorType) => {
    ...
    setValues((p) => ({ ...p, [type]: { highValue: d.high != null ? String(d.high) : "", highSeverity: "WARNING", lowValue: d.low != null ? String(d.low) : "", lowSeverity: "WARNING" } }));
    setSaved(false);
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {DATA_TYPES.map((type) => {
        ...
        return (
          <div key={type} style={{ background: "rgba(0,0,0,0.20)", backdropFilter: "blur(12px)", WebkitBackdropFilter: "blur(12px)", borderRadius: 24, overflow: "hidden" }}>
            {/* Card header */}
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, color: "#fff", minWidth: 0 }}>
                <meta.Icon size={14} strokeWidth={1.5} style={{ flexShrink: 0 }} />
                <span style={{ fontSize: 13, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{meta.label}</span>
              </div>
              <button
                type="button"
                onClick={() => reset(type)}
                title="Réinitialiser"
                style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 28, height: 28, color: "rgba(255,255,255,0.3)", background: "rgba(255,255,255,0.05)", border: "none", borderRadius: 8, cursor: "pointer", flexShrink: 0 }}
              >
                <RotateCcw size={12} />
              </button>
            </div>
            ...
            {/* High threshold row */}
            {hasHigh && (
              <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 16px", borderBottom: hasLow ? "1px solid rgba(255,255,255,0.05)" : "none" }}>
                <span style={{ color: "rgba(255,255,255,0.5)", fontSize: 11, width: 28, flexShrink: 0 }}>Haut</span>
                <NumberInput
                  value={v.highValue}
                  ...
                  step={type === "CO2" ? 50 : 1}
                />
                <SeveritySelect value={v.highSeverity} onChange={(val) => update(type, "highSeverity", val)} />
              </div>
            )}

            {/* Low threshold row */}
            {hasLow && (
              <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "9px 16px" }}>
                <span style={{ color: "rgba(255,255,255,0.5)", fontSize: 11, width: 28, flexShrink: 0 }}>Bas</span>
                <NumberInput
                  value={v.lowValue}
                  ...
                  step={type === "CO2" ? 50 : 1}
                />
                <SeveritySelect value={v.lowSeverity} onChange={(val) => update(type, "lowSeverity", val)} />
              </div>
            )}
          </div>
        );
  ...
  return (
    <IPhoneShell>
      <div style={{ position: "relative", minHeight: "100%", fontFamily: "var(--font-geist-sans), system-ui, -apple-system, sans-serif" }}>
        ...
        <div style={{ position: "relative", zIndex: 10, padding: "16px 16px 32px", display: "flex", flexDirection: "column", gap: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button
              type="button"
              style={{ padding: 8, borderRadius: 12, background: "rgba(255,255,255,0.08)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
            >
              <ArrowLeft size={18} style={{ color: "rgba(255,255,255,0.7)" }} />
            </button>
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              <h1 style={{ color: "#fff", fontSize: 18, fontWeight: 600, margin: 0 }}>Paramètres</h1>
              <p style={{ color: "rgba(255,255,255,0.4)", fontSize: 12, margin: 0 }}>Personnalisez les alertes et seuils</p>
            </div>
          </div>
